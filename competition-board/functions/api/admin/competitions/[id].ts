...
export const onRequestPut: PageFunction = async ({ env, request, params }) => {
  ...
  try {
    ...
    const result = await env.DB.prepare(
      `UPDATE competitions SET title = ?, category = ?, event_date = ?, location = ?, status = ?, description = ?, link = ?, featured = ?, updated_at = ? WHERE id = ?`,
    ).bind(competition.title, competition.category, competition.eventDate, competition.location, competition.status, competition.description, competition.link, competition.featured, now, params.id).run();
