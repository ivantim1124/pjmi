...
export const onRequestPost: PageFunction = async ({ env, request }) => {
  ...
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO competitions (id, title, category, event_date, location, status, description, link, featured, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).bind(id, competition.title, competition.category, competition.eventDate, competition.location, competition.status, competition.description, competition.link, competition.featured, now, now).run();
    return json({ id }, 201);
  } catch (error) {
    console.error(error);
